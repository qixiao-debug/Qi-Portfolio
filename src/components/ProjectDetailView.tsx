...
import { ArrowLeft, Check, Compass, ShieldCheck, Database, Calendar, BarChart2, TrendingUp, AlertCircle, HelpCircle, Clock, Target, Award, Layers, Zap, Lightbulb, ArrowDown, Link, Wrench, FileText, Globe, Megaphone, DollarSign, Search, TrendingDown, ExternalLink } from 'lucide-react';
import { motion } from 'motion/react';
import { Project } from '../types';
import { PROJECTS } from '../data';
import genshinUgreenBanner from '../assets/images/newsroom6.webp';
import { UgreenSeoPdfViewerModal } from './UgreenSeoPdfViewerModal';
  ...
  if (project.id === 'perf-marketing-seo') {
    return (
      <div className={`relative pt-20 md:pt-28 pb-20 min-h-screen text-left transition-colors font-sans ${
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="pb-10">
            <div className={`pl-5 md:pl-7 border-l-[5px] md:border-l-[6px] ${
              ...
              {/* Online PDF Portfolio Button */}
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <button
                  type="button"
                  onClick={() => setIsPdfModalOpen(true)}
                  className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold tracking-wide transition-all shadow-sm hover:shadow-md cursor-pointer hover:-translate-y-0.5 active:translate-y-0 ${
                    darkMode
                      ? 'bg-pink-600 hover:bg-pink-500 text-white shadow-pink-900/30'
                      : 'bg-pink-600 hover:bg-pink-700 text-white shadow-pink-500/25'
                  }`}
                  id="btn-seo-portfolio-pdf"
                >
                  <FileText className="w-4 h-4" />
                  <span>View Detailed Portfolio (PDF)</span>
                ...
                <div className={`text-xs font-mono flex items-center gap-1.5 ${
                  ...
                  Interactive PDF Portfolio & Playbook by Xiao Qi
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Interactive Full-Fidelity 13-Page PDF Presentation Viewer Modal */}
        <UgreenSeoPdfViewerModal
